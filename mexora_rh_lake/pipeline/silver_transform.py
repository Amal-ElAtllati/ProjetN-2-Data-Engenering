...
def normaliser_villes(df: pd.DataFrame) -> pd.DataFrame:
    def nettoyer_ville(v):
        if pd.isna(v): return "Non spécifié"
        v = str(v).lower().strip()
        if "casa" in v: return "Casablanca"
        if "rabat" in v: return "Rabat"
        if "tanger" in v: return "Tanger"
        if "marrakech" in v: return "Marrakech"
        if "fès" in v or "fes" in v: return "Fès"
        return v.title()

    df['ville_std'] = df['ville'].apply(nettoyer_ville)
    
    # Mapper sur la region
    regions = {
    ...
    df['region_admin'] = df['ville_std'].map(lambda x: regions.get(x, "Autre"))
    
    return df

def normaliser_contrat(df: pd.DataFrame) -> pd.DataFrame:
    def nettoyer_contrat(c):
        if pd.isna(c): return "Autre"
        c = str(c).lower().strip()
        if "cdi" in c or "indéterminée" in c or "permanent" in c: return "CDI"
        if "cdd" in c or "déterminée" in c: return "CDD"
        if "freelance" in c or "indépendant" in c: return "Freelance"
        if "stage" in c or "internship" in c: return "Stage"
        return "Autre"

    df['type_contrat_std'] = df['type_contrat'].apply(nettoyer_contrat)
    
    def extraire_dates(offre):
        date_pub = offre.get('date_publication', '')
        try:
            if '-' in date_pub:
                return date_pub[:4], date_pub[5:7]
            elif '/' in date_pub:
                return date_pub[6:], date_pub[3:5]
        except:
            pass
        return None, None
        
    df[['annee', 'mois']] = df.apply(lambda row: pd.Series(extraire_dates(row)), axis=1)
    return df
