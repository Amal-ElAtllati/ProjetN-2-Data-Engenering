...
def construire_gold(data_lake_root: str):
    ...
    # 2. Salaires par profil et ville
    ...
    df_salaires = con.execute(f"""
        ...
        -- HAVING COUNT(*) >= 5 (Commenté pour les données mockées pour être sûr d'avoir des résultats)
    ...
    # array_agg avec distinct en duckdb se fait avec list(distinct)
    df_entreprises = con.execute(f"""
        SELECT
            ...
            LIST(DISTINCT profil_normalise)    AS profils_recrutes,
